...
joblib.dump(clf, '../gbdt_feat/model/gbdt.model')
...
f = open("../gbdt_feat/first_gbdt_result.csv", "wb")
write = csv.writer(f)
write.writerows(pre)
f.close()
###############################################################################
...
print "saving model"
from sklearn.externals import joblib
joblib.dump(clf2, '../gbdt_feat/model/gbdt2.model')
...
##########################################################################################################
print "predicting"
feat_T=np.array(features,dtype="floate64")
feat_T1=clf.predict(feat_T)
second_feat_T=np.column_stack((feat_T,feat_T1))
pre2=clf2.predict(second_feat_T)
f = open("../gbdt_feat/second_gbdt_result.csv", "wb")
write = csv.writer(f)
write.writerows(pre2)
f.close()
####################################################################################################
f = open("../submit/two_gbdt_result.csv", "wb")
write = csv.writer(f)
write.writerow(["passengercount", "WIFIAPTag","slice10min"])
for i in range(len(pre)):
    pre_date="2016-09-14-"
    wifiname=wifi_name_dict[int(second_feat_T[i][0])]
    slice10h=15+int(second_feat_T[i][1]-1)/6
    slice10m=int((second_feat_T[i][1]-1)%6)
