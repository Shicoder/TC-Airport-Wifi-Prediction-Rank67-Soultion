...
clf=GradientBoostingRegressor(n_estimators=1500,learning_rate=0.01,max_depth=20)
...
for file_name in file_list:
    ...
    for i in range(18):
        ...
        key=wifi_name_dict[int(file_name.split(".")[0])]+"-"+time
        mean=hour1_mean_dict[key]
...
####################################################################################################
f = open("../submit/two_gbdt_result_4_m.csv", "wb")
write = csv.writer(f)
