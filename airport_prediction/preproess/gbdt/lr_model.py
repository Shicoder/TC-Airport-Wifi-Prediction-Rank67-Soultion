...
"""???????????wifi??????��???��??????????????????????????????????
???????11??????12??????1??????2??????
6-8?????9-11?????12-2??????6-14??????6-8??9-11??9-11??12-2??6-8??12-2??1??????1??��???3???3??��??Y?��??????"""
...
clf2.fit(feat, la)
from sklearn.externals import joblib
joblib.dump(clf2, '../gbdt_feat/model/svm.model')
lr = joblib.load('../gbdt_feat/model/svm.model')
# for m in range(len(model.feature_importances_)):
#     if model.feature_importances_[m]>0.05:
#         print "feature_importance",m,model.feature_importances_[m]
test = np.loadtxt(open("../gbdt_feat/SPSS_feature_pro.csv","rb"),delimiter=",",skiprows=0)
...
#####################
f = open("../submit/svm_result.csv", "wb")
write = csv.writer(f)
