...
for file_name in file_list:
    ...
    for row in rows:
            ...
            if int(time) >= 15 and int(time) <=17 and int(date)>=17 and int(date)<=23:
                ...
                #######################################################
                feature=[int(file_name.split(".")[0]),time_series,simple_hour_list[1],simple_hour_list[2],
...
test = np.loadtxt(open("../gbdt_feat/gbdtpjf_feat.csv","rb"),delimiter=",",skiprows=0)
import numpy as np
print "predicting"
test=np.array(test)
...
f = open("../submit/gbdtpjf_feat_result.csv", "wb")
write = csv.writer(f)
