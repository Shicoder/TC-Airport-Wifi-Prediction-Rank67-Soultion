...
train_data = np.loadtxt(open("../gbdt_feat/xgboost_feat_train.csv","rb"),delimiter=",",skiprows=0)
# train_data = pd.read_csv("../gbdt_feat/xgboost_feat_train_wight_mean_add_quyurenshu.csv",sep=',',names=range(31))
train_data=np.array(train_data)
print  train_data.shape
train_data=np.asarray(train_data, dtype=np.float64)
train_target = np.loadtxt(open("../gbdt_feat/xgboost_train_lable.csv","rb"),delimiter=",",skiprows=0)
...
train_data=np.asarray(train_data, dtype=np.float64)
#########################################################
test_data = np.loadtxt(open("../gbdt_feat/xgboost_target.csv","rb"),delimiter=",",skiprows=0)
...
def daochu(pred):
    listpred=[]
    f = open('../gbdt_feat/xgboost_target.csv')
    num=0
    rows = csv.reader(f)
    for row in rows:
    ...
    result.to_csv("../train/reslut_xgboost_10_22.csv", index=False)
