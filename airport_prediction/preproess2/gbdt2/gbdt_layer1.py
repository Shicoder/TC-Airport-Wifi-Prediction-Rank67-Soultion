...
for file_name in file_list:
    ...
    for row in rows:
            ...
            if int(time) >= 15 and int(time) <=17 and int(date)>=19 and int(date)<=22:
...
########################################################
                ...
                #########################################
                key = wifi_name_dict[int(file_name.split(".")[0])]+"-"+str(date)+"-"+str(time_series)
                print sd_win13_dict["WC-3C<WC-3-13>-20-9"]
                s1 = sd_win13_dict[key]
                ###################    feature   ################
...
# test = np.loadtxt(open("../gbdt_feat/gbdt_layer1_feat.csv","rb"),delimiter=",",skiprows=0)
#########################################################
for file_name in file_list:
    ...
    for row in rows:
            ...
            time = row[1].split(" ")[1].split(":")[0]
            minute = row[1].split(" ")[1].split(":")[1][0]
            if int(time) >= 15 and int(time) <=17 and int(date)==23:
                wifi_name=wifi_name_dict[int(file_name.split(".")[0])]
                simple_hour_name_pre =wifi_name+":"+date+"-"
                #####################
                hou_mean_6_14=(float(simple_hour_dict[simple_hour_name_pre+"6"])+float(simple_hour_dict[simple_hour_name_pre+"7"])+float(simple_hour_dict[simple_hour_name_pre+"8"])+\
...
########################################################
                ...
                lable=row[0]
                #########################################################
                second_features.append(feature)
                second_lables.append(lable)
###########################################################
import numpy as np
print "predicting"
feat=np.array(second_features)
s_la=np.array(second_lables)
feat=np.asarray(feat, dtype=np.float64)
s_la=np.asarray(s_la, dtype=np.float64)
pre=clf2.predict(feat)

############################################
second_feat=np.column_stack((feat,pre))
feat=np.array(second_feat)
feat=np.asarray(feat, dtype=np.float64)
clf3=GradientBoostingRegressor(n_estimators=1000,learning_rate=0.1,max_depth=20,max_features=10)
print "starting second fitting"
clf3.fit(feat,s_la)
#################################################
print "loading test data"
test = np.loadtxt(open("../gbdt_feat/gbdt_layer1_feat.csv","rb"),delimiter=",",skiprows=0)
...
second_feat=np.column_stack((test,pre))
feat=np.array(second_feat)
feat=np.asarray(feat, dtype=np.float64)
pre=clf3.predict(feat)
###############################################
f = open("../submit/two_gbdt_layer1_2.csv", "wb")
write = csv.writer(f)
