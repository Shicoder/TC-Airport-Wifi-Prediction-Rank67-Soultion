...
for file_name in file_list:
    ...
    for row in rows:
            ...
            if int(time) >= 15 and int(time) <=17 and int(date)>=21 and int(date)<=24:
...
test = np.loadtxt(open("../gbdt_feat/gbdt2_2_class_feat.csv","rb"),delimiter=",",skiprows=0)
import numpy as np
print "predicting"
pre=clf2.predict(test)

f = open("../submit/gbdt2_2_result_classifer.csv", "wb")
write = csv.writer(f)
